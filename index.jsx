...
export const render = ({name, weather, time, reminders}, dispatch) => {
  return (
    <div className={css({height: "100%", padding: "0px 5px",display:"flex", flexDirection: "column", backgroundImage: `url("${current_background}")`, backgroundSize: "cover"})}>
      <Header>
      ...
      <Main>
        <DateContainer>
          <p className={`${css({marginTop: "10px"})} ${text}`}>{actions.getDate()}</p>
          <p className={`${text}`}>{weather ? weather.description : "inconnu"}</p>
          <p className={`${css({margin: "10px 0", fontWeight: 350, fontSize: "20px"})} ${text}`}>{time}</p>
        </DateContainer>
        <ToDoContainer>
